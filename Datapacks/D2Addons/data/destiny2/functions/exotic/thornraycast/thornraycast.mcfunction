...
execute as @e[type=!#timelesscraft:not_mob,tag=!thornraycasting,dx=0] positioned ~-0.99 ~-0.99 ~-0.99 if entity @s[dx=0] positioned ~0.99 ~0.99 ~0.99 run function destiny2:exotic/thornraycast/thornraycastcollide

## .distance can be any fake player name
scoreboard players add .distance thornrcast 1
...
execute if score .distance thornrcast matches ..300 positioned ^ ^ ^0.1 rotated ~ ~ if block ~ ~ ~ #timelesscraft:raycast_pass run function destiny2:exotic/thornraycast/thornraycast
