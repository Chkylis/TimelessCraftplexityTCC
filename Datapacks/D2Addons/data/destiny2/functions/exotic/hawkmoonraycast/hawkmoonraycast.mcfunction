...
execute as @e[type=!#timelesscraft:not_mob,tag=!hawkmoonraycasting,dx=0] positioned ~-0.99 ~-0.99 ~-0.99 if entity @s[dx=0] positioned ~0.99 ~0.99 ~0.99 run function destiny2:exotic/hawkmoonraycast/hawkmoonraycastcollide

## .distance can be any fake player name
scoreboard players add .distance hawkmoonrcast 1
...
execute if score .distance hawkmoonrcast matches ..300 positioned ^ ^ ^0.1 rotated ~ ~ if block ~ ~ ~ #timelesscraft:raycast_pass run function destiny2:exotic/hawkmoonraycast/hawkmoonraycast
